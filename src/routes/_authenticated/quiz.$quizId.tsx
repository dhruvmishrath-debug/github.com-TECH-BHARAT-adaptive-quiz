  ...
  head: ({ params }) => ({
    meta: [
      { title: "Take Quiz — QuizGen" },
      {
        name: "description",
        content:
          "Answer your AI-generated quiz one question at a time, then submit to get instant scoring and explanations for every answer.",
      },
      { property: "og:title", content: "Take Your QuizGen Quiz" },
      {
        property: "og:description",
        content: "Adaptive multiple-choice quiz with instant scoring and answer explanations.",
      },
      { property: "og:url", content: `https://lnct-tech-bharat.lovable.app/quiz/${params.quizId}` },
      { name: "robots", content: "noindex" },
    ],
    links: [
      { rel: "canonical", href: `https://lnct-tech-bharat.lovable.app/quiz/${params.quizId}` },
    ],
  }),
  component: QuizPage,
});
...
function QuizPage() {
  ...
  const [quiz, setQuiz] = useState<{ title: string; difficulty: string; subject: string } | null>(
    null,
  );
  ...
  const opts = useMemo(
    () => (current ? [current.option_a, current.option_b, current.option_c, current.option_d] : []),
    [current],
  );
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const { attemptId } = await submit({
        data: { quizId, answers: payload, timeTakenSeconds: elapsed },
      });
        ...
        <div className="mt-6 space-y-2.5">
          {opts.map((opt, i) => {
            ...
            return (
              ...
              >
                <span
                  className={`grid h-7 w-7 shrink-0 place-items-center rounded-lg font-bold ${
                    isSelected
